(() => {
  ...
  */
  ...
  function ensureStandardButton(){
    /* V29: il pulsante non viene più mostrato nella sidebar. */
  }

  function ensureInsideStandardButton(card){
    if(!card) return;

    let row = card.querySelector('.v29-standard-row');
    if(row) return;

    row = document.createElement('div');
    row.className = 'v29-standard-row';

    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'v29-standard-inside';
    ...
    button.addEventListener('click', event => {
      ...
      document.body.classList.remove('v29-expanded-active');
  ...
  function clearExpansion(card){
    card.classList.remove('v29-expanded');
  ...
  function applyExpandedLayout(){
    ...
    document.body.classList.add('v29-expanded-active');
    selected.classList.add('v29-expanded');
