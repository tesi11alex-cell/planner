(() => {
  ...
  */
  ...
  function ensureStandardButton(){
    /* V30: il pulsante non viene più mostrato nella sidebar. */
  }

  function ensureInsideStandardButton(card){
    if(!card) return;

    let row = card.querySelector('.v30-standard-row');
    if(row) return;
    ...
    row.className = 'v30-standard-row';

    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'v30-standard-inside';
    button.textContent = '↺ Vista standard';
    button.title = 'Torna alla vista standard';

    row.appendChild(button);

    /* V30: il pulsante entra DENTRO l'intestazione del giorno,
    ...
    button.addEventListener('click', event => {
      ...
      document.body.classList.remove('v30-expanded-active');
  ...
  function clearExpansion(card){
    card.classList.remove('v30-expanded');
  ...
  function applyExpandedLayout(){
    ...
    document.body.classList.add('v30-expanded-active');
    selected.classList.add('v30-expanded');
