  ...
  */
  function applyCompactHoursToCard(card){
    ...
    card.querySelectorAll('.hour-row[data-base-hour]').forEach(row => {
      ...
      const keepBoundary = ['08:00','11:00','14:00','17:00','20:00'].includes(baseHour);
  ...
  function ensureStandardButton(){
    /* V28: il pulsante non viene più mostrato nella sidebar. */
  }

  function ensureInsideStandardButton(card){
    if(!card) return;

    let button = card.querySelector('.v28-standard-inside');
    if(button) return;

    button = document.createElement('button');
    button.type = 'button';
    button.className = 'v28-standard-inside';
    ...
    button.addEventListener('click', event => {
      ...
      document.body.classList.remove('v28-expanded-active');
  ...
  function clearExpansion(card){
    card.classList.remove('v28-expanded');
  ...
  function applyExpandedLayout(){
    ...
    document.body.classList.add('v28-expanded-active');
    selected.classList.add('v28-expanded');
