(() => {
  ...
  */
  ...
  function setStandardView(options={}){
    ...
    document.body.classList.remove('v32-expanded-active');
    document.body.classList.remove('v32-notes-expanded-active');
...
function ensureStandardButton(){
    /* V32: il pulsante non viene più mostrato nella sidebar. */
  }

  function ensureInsideStandardButton(card){
    if(!card) return;

    let row = card.querySelector('.v32-standard-row');
    if(row) return;
    ...
    row.className = 'v32-standard-row';

    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'v32-standard-inside';
  ...
  function ensureNotesActions(){
    ...
    if(head.querySelector('.v32-notes-actions')) return;

    const actions = document.createElement('div');
    actions.className = 'v32-notes-actions';

    const standardButton = document.createElement('button');
    standardButton.type = 'button';
    standardButton.className = 'v32-notes-standard';
  ...
  function applyNotesLayout(){
    ...
    notes.classList.remove('v32-notes-expanded');
    ...
    document.body.classList.add('v32-notes-expanded-active');
    document.body.classList.remove('v32-expanded-active');
    notes.classList.add('v32-notes-expanded');
  ...
  function clearExpansion(card){
    card.classList.remove('v32-expanded');
  ...
  function applyExpandedLayout(){
    ...
    document.body.classList.add('v32-expanded-active');
    selected.classList.add('v32-expanded');
