  ...
  */
  function applyCompactHoursToCard(card){
    ...
    card.querySelectorAll('.hour-row[data-base-hour]').forEach(row => {
      ...
      const keepBoundary = ['08:00','12:00','16:00','20:00'].includes(baseHour);
  ...
  function ensureStandardButton(){
    /* V27: il pulsante non viene più mostrato nella sidebar. */
  }

  function ensureInsideStandardButton(card){
    if(!card) return;

    let button = card.querySelector('.v27-standard-inside');
    if(button) return;

    button = document.createElement('button');
    button.type = 'button';
    button.className = 'v27-standard-inside';
    ...
    button.addEventListener('click', event => {
      ...
      document.body.classList.remove('v27-expanded-active');
  ...
  function clearExpansion(card){
    card.classList.remove('v27-expanded');
  ...
  function applyExpandedLayout(){
    ...
    document.body.classList.add('v27-expanded-active');
    selected.classList.add('v27-expanded');
